...
class U2FEnrollResponseSchema(EduidSchema):
    ...
    registered_keys = fields.Nested(
        U2FRegisteredKey, required=True, missing=list(), load_from='registeredKeys', dump_to='registeredKeys', many=True
    )
    register_requests = fields.Nested(
        U2FRegisterRequest, required=True, load_from='registerRequests', dump_to='registerRequests', many=True
    )
...
class U2FBindRequestSchema(EduidSchema):
    ...
    version = fields.String(required=True)
    registration_data = fields.String(required=True, load_from='registrationData', dump_to='registrationData')
    client_data = fields.String(required=True, load_from='clientData', dump_to='clientData')
